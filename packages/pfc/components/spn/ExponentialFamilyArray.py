...
class ExponentialFamilyArray(torch.nn.Module):
    ...
    def __init__(self, num_var, num_dims, array_shape, num_stats, use_em, **kwargs):
        ...
        if not self._use_em:
            ...
            def _retain_grad_hook(module, _inp, out):
                if out.requires_grad:
                    try:
                        out.retain_grad()
                    except RuntimeError:         # happens only under functorch
                        pass
                module.ll = out                # keep handle for EM code
            self.register_forward_hook(_retain_grad_hook)

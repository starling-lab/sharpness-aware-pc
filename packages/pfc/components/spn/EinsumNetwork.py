...
class EinsumNetwork(torch.nn.Module):
    ...
    def em_process_batch(self, reg_lam=0.0, reg_mu=0.0):
        for i,l in enumerate(self.einet_layers):
            if type(reg_lam) == list and i>0:
                reg_lam_i = reg_lam[i-1] if i < len(reg_lam) else 0.0
                l.em_process_batch(reg_lam=reg_lam_i, reg_mu=reg_mu)
            else:
                l.em_process_batch(reg_lam=reg_lam, reg_mu=reg_mu)

    def em_update(self, reg_lam=0.0, reg_mu=0.0):
        for i,l in enumerate(self.einet_layers):
            if type(reg_lam) == list and i>0:
                reg_lam_i = reg_lam[i-1] if i < len(reg_lam) else 0.0
                l.em_update(reg_lam=reg_lam_i, reg_mu=reg_mu)
            else:
                l.em_update(reg_lam=reg_lam, reg_mu=reg_mu)

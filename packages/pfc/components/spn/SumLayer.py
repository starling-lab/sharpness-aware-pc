...
class SumLayer(Layer):
    ...
    def em_process_batch(self, reg_lam=0.0, reg_mu=0.1):
        """
        ...
        if self.online_em_frequency is not None:
            ...
            if self._online_em_counter == self.online_em_frequency:
                self.em_update(True, reg_lam, reg_mu)
                self._online_em_counter = 0

    def em_update(self, _triggered=False, reg_lam=0.0, reg_mu=0.1):
        """
        ...
        with torch.no_grad():
            if type(reg_lam) != torch.Tensor and reg_lam == 0:
                n = self.params.grad * self.params.data

            elif  type(reg_lam) == torch.Tensor or  reg_lam > 0:
                # n = self.params.data
                n = self.params.grad * self.params.data
                n = (n + torch.sqrt(n**2 + 4*reg_lam*reg_mu*n)) / (2*reg_mu)
                # print(reg_lam, reg_mu)
                # n = n*((1+torch.sqrt(n**2 + 4*reg*reg_mu))/2*reg)
                # for _ in range(5):
                #     n = n * torch.sqrt((1 + 2*reg/(self.params.grad*self.params.data.pow(2))))
                # print("Difference between updates:", ((n + torch.sqrt(n**2 + 4*reg_lam*reg_mu*n)) / (2*reg_lam)).norm())
    ...
    def project_params(self):
        ...
        self.params.data = self.reparam(self.params.data)

...
from packages.pyjuice.nodes import SumNodes
from packages.pyjuice.utils import BitSet
from packages.pyjuice.utils.parameter_list import FastParamList
from packages.pyjuice.utils.kernel_launcher import FastJITFunction
...
class SumLayer(Layer, nn.Module):
    ...
    def _forward_pytorch_kernel(node_mars: torch.Tensor, element_mars: torch.Tensor, params: torch.Tensor, 
        ...
        if propagation_alg_id == 0:
            ...
            node_mars[nids] = (ch_mars + params[pids].clamp(min = 1e-10).log().unsqueeze(-1)).max(dim = 1).values
    ...
    def _backward_pytorch_ele_kernel(self, node_flows: torch.Tensor, element_flows: torch.Tensor, 
        ...
        if logspace_flows:
            if accumulate_ch_flows:
                element_flows[chids] += (node_flows[parids] + params[parpids].clamp(min=1e-10).log().unsqueeze(-1) + \

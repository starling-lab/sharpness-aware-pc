...
from packages.pyjuice.nodes import CircuitNodes
...
def sgd_par_update_kernel(params, param_grads, par_start_ids, pgrad_start_ids, blk_sizes, blk_intervals,
    ...
    if keep_zero_params:
        # updated_param = tl.where(old_param < 1e-12, 0.0, tl.exp(tl.log(old_param) + lr * pgrads))
        updated_param = tl.where(old_param < 1e-12, 0.0, old_param + lr * pgrads)
    else:
        # updated_param = tl.exp(tl.log(old_param) + lr * pgrads) #old_param + lr * pgrads
        updated_param = old_param + lr * pgrads

...
from packages.pyjuice.nodes import CircuitNodes, InputNodes, ProdNodes, SumNodes, foreach, summate, multiply
from packages.pyjuice.layer import Layer, InputLayer, ProdLayer, SumLayer, LayerGroup
from packages.pyjuice.utils.grad_fns import ReverseGrad
from packages.pyjuice.utils import BitSet
...
from .backend import compile_cum_par_flows_fn, compute_cum_par_flows, cum_par_flows_to_device, \
                     ...
                     normalize_parameters, eval_top_down_probs, sgd_par_update
...
class TensorCircuit(nn.Module):
    ...
    def mini_batch_em(self, step_size: float, pseudocount: float = 0.0, keep_zero_params: bool = True,
                      step_size_rescaling: bool = False, reg_mu: float=0.0, reg_lam: float=0.0):
        """
        ...
        if type(reg_mu)==torch.Tensor or reg_mu != 0:
            self.param_flows = (self.param_flows + torch.sqrt(self.param_flows**2 + 4*reg_mu*reg_lam*self.param_flows))/2*reg_lam
    ...
    def sgd_update(self, inputs: torch.Tensor, lr: float, ll_weights: Optional[torch.Tensor] = None,
                   reg_coeff: float = 0.0, keep_zero_params: bool = True):
        """Perform a single SGD update using ``sgd_par_update``.

        The gradients are obtained via :func:`compute_param_grads`. When
        ``reg_coeff`` is non-zero, a negative squared gradient regularization is
        applied using the Hessian diagonal approximation ``-(grad ** 2)``.
        ...
        :param reg_coeff: coefficient for the negative squared gradient
            regularization term. Set to 0 for vanilla SGD.
        :type reg_coeff: float

        :param keep_zero_params: whether to freeze parameters that are exactly
            zero
        :type keep_zero_params: bool
        """
        ...
        with torch.no_grad():
            grads = self.compute_param_grads(inputs, ll_weights = ll_weights)
            
            # Accumulate parameter flows of tied nodes
            compute_cum_par_flows(self.param_flows, self.parflow_fusing_kwargs)
            # Update input layers
            for layer in self.input_layer_group:
                layer.mini_batch_em(step_size = lr, pseudocount = 0.0)
            
            if reg_coeff != 0.0:
                # grad_p = grads*self.params[self.num_dummy_params:]
                # hessian = torch.outer(grads, grads)
                trace = self.compute_hessian_trace(inputs, ll_weights = ll_weights)
                grads = grads * (1  -  2*reg_coeff*trace)
                # grads = grads * (1 - 2 * reg_coeff * hessian.sum(-1))

            sgd_par_update(
                self.params,
                grads,
                self.par_update_kwargs,
                lr = lr,
                keep_zero_params = keep_zero_params,
            )

        self._normalize_parameters(self.params)
        return None
    ...
    def compute_param_grads(self, inputs: torch.Tensor, ll_weights: Optional[torch.Tensor] = None):
        """Compute gradients of the log-likelihood with respect to the log parameters.

        This runs a forward and backward pass with ``compute_param_flows=True`` and
        returns ``self.param_flows / self.params``.

        :param inputs: input tensor of size ``[B, num_vars]``
        :type inputs: torch.Tensor

        :param ll_weights: optional weights of the log-likelihoods
        :type ll_weights: Optional[torch.Tensor]
        """

        with torch.no_grad():
            self.forward(inputs)
            self.backward(inputs = inputs, ll_weights = ll_weights, compute_param_flows = True, flows_memory = 1.0)
        self.cumulate_flows(inputs)
        
        grads = self.param_flows/self.params[self.num_dummy_params:]
        return grads
        # return self.param_flows
        
    def compute_hessian_diag(self, inputs: torch.Tensor, ll_weights: Optional[torch.Tensor] = None):
        """Return the diagonal of the Hessian matrix of the log-likelihood.

        This simply computes ``-(grads ** 2)`` where ``grads`` are obtained from
        :func:`compute_param_grads`.
        """

        grads = self.compute_param_grads(inputs, ll_weights = ll_weights)
        return -(grads ** 2)

    def compute_hessian_trace(self, inputs: torch.Tensor, ll_weights: Optional[torch.Tensor] = None):
        """Return the trace of the Hessian matrix of the log-likelihood."""
        diag = self.compute_hessian_diag(inputs, ll_weights = ll_weights)
        return diag.sum().abs()/len(inputs)
